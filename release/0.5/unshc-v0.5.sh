...
function clean(){
	$SHRED -zu -n 1 $OBJFILE $CALLFILE $CALLADDRFILE $CALLSIZEFILE $STRINGFILE $TMPBINARY ${TMPBINARY}.c >/dev/null 2>&1
}
...
function extract_arc4_call_addr(){
	CALLADDR=$($GREP -Eo "call.*[0-9a-f]{6,}" $OBJFILE | $GREP -Eo "[0-9a-f]{6,}" | $SORT | $UNIQ -c | $SORT | $GREP -Eo "(14).*[0-9a-f]{6,}" | $GREP -Eo "[0-9a-f]{6,}" | tail -n 1)
	if [ -z "$CALLADDR" ]; then
