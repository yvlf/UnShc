...
function extract_variables_from_binary(){
 ...
 do
  ...
  echo -n ":- Working with var address: $i "
...
function extract_password_from_binary(){
 ...
 if [ "x$($GREP movl $CALLFILE)" != "x" ]
 then
  $GREP -B 7 -m 1 "call   $CALLADDR" $OBJFILE | $GREP -v $CALLADDR > $CALLFILE
 else
 ...
 if [ "x$($GREP "movl" $CALLFILE)" != "x" ]
 then
  KEY_ADDR=$($GREP -B 1 -m 1 "call" $CALLFILE | $GREP movl | $GREP -oE "0x[0-9a-z]+")

  # Discovering the key size.
  KEY_SIZE=$($GREP -B 3 -m 1 "call" $CALLFILE | $GREP movl | head -n 1 | $GREP -oE "0x[0-9a-z]+," | $GREP -oE "0x[0-9a-z]+")
 else
